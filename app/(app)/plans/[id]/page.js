...
export default async function PlanDetailPage({ params, searchParams }) {
  ...
  // findUnique(id)만으로 찾으면 남의 계획도 존재만 하면 걸려 나온다.
  // 반드시 userId까지 같이 조건을 걸어야 "내 것"만 찾을 수 있다. 없으면(=
  // 아예 없거나 남의 것이면) 404로 존재 자체를 감춘다 (T07-C121).
  ...
  const review = await computeReview(plan.id, user.id);
